...
if ! grep -F "Pkg.Revision = 25.2" $NDK/source.properties; then
    echo NDK version should be r25c
fi
gdb_prefix=/opt/gdb
GDB_VERSION=13.2
PYTHON_VERSION=3.10
...
sudo apt-get install gcc-11-arm-linux-gnueabihf cpp-arm-linux-gnueabihf gcc-arm-linux-gnueabihf g++-arm-linux-gnueabihf || exit 1
...
mkdir zlib && cd zlib && ../../zlib/configure  \
    ...
    make && cd .. || exit 1
mkdir libsframe && cd libsframe && ../../libsframe/configure  \
        PATH=$NDK/toolchains/llvm/prebuilt/linux-x86_64/bin/:$PATH \
        --host=aarch64-linux-android --program-prefix=aarch64-linux-android- \
        LDFLAGS="-static" \
        CPPFLAGS="-DHAVE_ELF32_AUXV_T -DHAVE_ELF64_AUXV_T" \
...
mkdir build-gdbserver-arm-linux-gnueabihf && cd build-gdbserver-arm-linux-gnueabihf || exit 1
mkdir zlib && cd zlib && ../../zlib/configure  \
        LDFLAGS=-static --host=arm-linux-gnueabihf \
...
../gdbserver/configure CPPFLAGS="-include gnulib/import/stdio.h"  \
    ...
    sed -i 's/LIBIBERTY_BUILDDIR = ..\/libiberty/LIBIBERTY_BUILDDIR = libiberty/g' Makefile && \
    sed -i 's/GDBSUPPORT_BUILDDIR = ..\/gdbsupport/GDBSUPPORT_BUILDDIR = gdbsupport/g' Makefile && \
    make gdbserver && arm-linux-gnueabihf-strip gdbserver && cd .. || exit 1
