                                       ...
                                       (let (
                                         ...
                                         (when has-match
                                           (if (null idx)
                                               (setq idx 0)
                                             (cl-incf idx))
                                           (when (if (>= (swiper--get-begin x)
                                                         swiper--opoint)
                                                     (and (null first-past-opoint-idx)
                                                          (not swiper--async-direction-backward))
                                                   swiper--async-direction-backward)
...
(defun ivy-previous-line-hook(orig-fun &optional arg)
  (interactive "p")
  (when (eq (ivy-state-caller ivy-last) 'swiper-async)
    (setq swiper--async-direction-backward t))
  (funcall orig-fun arg))
(defun ivy-next-line-hook(orig-fun &optional arg)
  (interactive "p")
  (when (eq (ivy-state-caller ivy-last) 'swiper-async)
