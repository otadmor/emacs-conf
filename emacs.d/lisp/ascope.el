...
"The keymap used in the *Completions** buffer which lists search results.")
...
(defvar ascope-output-buffer-name "*Completions*"
...
;(message "buffer is %S" (get-text-property (point) 'ascope-file))
;(set-frame-selected-window nil (display-buffer (get-text-property (point) 'ascope-file)))
...
"The body of the following four functions."
    ...
    (winstack-push old-buffer-window nil)
    (set-buffer buffer)
    (setq old-point (point))
...
(with-current-buffer ascope-output-buffer-name
